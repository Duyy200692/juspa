  ...
  return (
    <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-pink-100 transform hover:scale-[1.01] transition-transform duration-300 relative group">
      <div className="p-6 bg-gradient-to-br from-[#FDF7F8] to-white flex justify-between items-start">
        ...
        {canEdit && (
            <Button variant="secondary" onClick={onEdit} className="text-xs px-2 py-1 opacity-0 group-hover:opacity-100 transition-opacity">
                Edit
            </Button>
        )}
      </div>
          ...
          {services.map(service => {
            ...
            return (
              <li key={service.id} className="border-t border-dashed border-pink-200 py-3 flex items-center px-4">
                <div className="w-2/5">
                  <p className="font-bold text-sm text-[#5C3A3A]">
                      {service.name}
                      {service.isCombo && <span className="ml-2 text-[10px] bg-purple-100 text-purple-600 px-1 rounded">COMBO</span>}
                  </p>
