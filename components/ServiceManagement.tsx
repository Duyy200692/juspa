                                ...
                                <tr key={service.id}>
                                    {editingServiceId === service.id ? (
                                        ...
                                        <>
                                            ...
                                            <td className="py-4 px-6 whitespace-nowrap text-sm text-gray-700 text-right">{formatCurrency(service.pricePerSession)}</td>
                                            <td className="py-4 px-6 whitespace-nowrap text-sm text-gray-700 text-right">{formatCurrency(service.priceFullPackage)}</td>
                                            <td className="py-4 px-6 whitespace-nowrap text-center space-x-2">
                                                ...
                                                <Button variant="danger" onClick={() => window.confirm(`Are you sure you want to delete ${service.name}?`) && onDeleteService(service.id)}>Delete</Button>
                                            </td>
                                        </>
                                    )}
                                </tr>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
                 ...
                 <form onSubmit={handleAddNewService} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                    <div className="lg:col-span-2">
                        ...
                        <input type="text" value={newService.name} onChange={e => handleNewServiceChange('name', e.target.value)} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" required/>
                    </div>
                     <div>
                    ...
                    <div className="md:col-span-2 lg:col-span-4">
                         ...
                         <textarea value={newService.description} onChange={e => handleNewServiceChange('description', e.target.value)} rows={2} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2"></textarea>
                    </div>
